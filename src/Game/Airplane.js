import { Group, BoxGeometry, BufferGeometry, BufferAttribute, DoubleSide, MeshPhongMaterial, Mesh, Color, Matrix4 } from 'three';
...
export default class Airplane extends Group {
  constructor() {
    ...
    const vertices = new Float32Array(
      this.makeTetrahedron(frontUL, frontUR, frontLL, frontLR).concat(   // front
      this.makeTetrahedron(backUL, backUR, backLL, backLR)).concat(      // back
      this.makeTetrahedron(backUR, backLR, frontUR, frontLR)).concat(    // side
      this.makeTetrahedron(backUL, backLL, frontUL, frontLL)).concat(    // side
      this.makeTetrahedron(frontUL, backUL, frontUR, backUR)).concat(    // top
      this.makeTetrahedron(frontLL, backLL, frontLR, backLR))            // bottom
    );

    const geomCabin = new BufferGeometry();
    geomCabin.setAttribute('position', new BufferAttribute(vertices, 3));

    var matCabin = new MeshPhongMaterial({color: new Color('red'), flatShading: true, side: DoubleSide});
    ...
    var matEngine = new MeshPhongMaterial({color: new Color('white'), flatShading: true});
    ...
    var matTailPlane = new MeshPhongMaterial({color: new Color('red'), flatShading: true});
    ...
    var matSideWing = new MeshPhongMaterial({color: new Color('red'), flatShading: true});
    ...
    var matPropeller = new MeshPhongMaterial({color: new Color('brown'), flatShading: true});
    ...
    var matBlade = new MeshPhongMaterial({color: new Color('black'), flatShading: true});
    ...
    var wheelProtecMat = new MeshPhongMaterial({color: new Color('red'), flatShading: true});
    ...
    var wheelTireMat = new MeshPhongMaterial({color: new Color('black'), flatShading: true});
    var wheelTireR = new Mesh(wheelTireGeom,wheelTireMat);
    wheelTireR.position.set(25,-28,25);

    var wheelAxisGeom = new BoxGeometry(10,10,6);
    var wheelAxisMat = new MeshPhongMaterial({color: new Color('brown'), flatShading: true});
    ...
    var suspensionMat = new MeshPhongMaterial({color: new Color('red'), flatShading: true});
    ...
    var matWindshield = new MeshPhongMaterial({color: new Color('white'), transparent:true, opacity:.3, flatShading: true});
